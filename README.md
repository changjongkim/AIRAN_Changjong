...
### Exp-19: MIG-like 40:60 + Qwen-32B/72B NVLink Tensor Parallel (2026-04-14)

**실제 AI-RAN 구조 재현**: MIG-like SM 격리(40:60) + 대형 LLM이 4GPU NVLink TP로 분산.
각 GPU에서 L1(SM 40%) + Qwen shard(SM 60%)가 MPS로 공존.

```
각 노드:
  GPU0: L1(SM 40%) + Qwen shard 0(SM 60%) ─┐
  GPU1: L1(SM 40%) + Qwen shard 1(SM 60%) ─┤ NVLink tensor parallel
  GPU2: L1(SM 40%) + Qwen shard 2(SM 60%) ─┤ (매 inference마다 all-reduce)
  GPU3: L1(SM 40%) + Qwen shard 3(SM 60%) ─┘
```

#### Qwen-32B (40GB, ~16GB/GPU) — 1N/2N/4N

| Scale | Baseline | + Qwen-32B TP (range) | 최대 간섭 | TTI miss |
|-------|---------|----------------------|----------|---------|
| **1N (4GPU)** | 0.593ms | 0.470~0.503ms | **~1.0x** | **0%** |
| **2N (8GPU)** | 0.468ms | 0.468~0.542ms | **~1.0x** | **0%** |
| **4N (16GPU)** | 0.468ms | 0.466~0.542ms | **~1.0x** | **0%** |

**32B: 모든 스케일에서 간섭 0, TTI miss 0.**
모델이 작아서 (16GB/GPU, HBM 38%) bandwidth 압박이 없음.
NVLink tensor parallel 통신도 L1에 영향 없음.

#### Qwen-72B (80GB, ~36GB/GPU) — 1N/2N/4N

| Scale | Baseline | + Qwen-72B TP (range) | 최대 간섭 | TTI miss (worst GPU) |
|-------|---------|----------------------|----------|---------------------|
| **1N (4GPU)** | 0.518ms | 0.576~0.629ms | **1.21x** | **2%** |
| **2N (8GPU)** | 0.467ms | 0.495~0.931ms | **1.99x** | **8%** |
| **4N (16GPU)** | 0.469ms | 0.505~0.900ms | **1.92x** | **10%** |

**72B: 노드 스케일 시 worst-case GPU의 간섭이 증가.**

```
1N → 2N → 4N 변화:
  최대 간섭: 1.21x → 1.99x → 1.92x (증가 후 안정)
  TTI miss:  2%   → 8%   → 10%  (점진적 증가)
  
원인 분석:
  72B = GPU당 36GB / 80GB = HBM 45% 사용
  + L1 pipeline HBM → 총 HBM ~50%+
  + NVLink all-reduce가 매 inference마다 추가 bandwidth 소모
  → HBM bandwidth 경쟁이 발생하여 L1 latency 증가
```

**GPU 간 간섭 불균형 (72B 4N 예시, nid008633):**

| GPU | + Qwen-72B TP | vs baseline | miss |
|-----|--------------|-------------|------|
| GPU0 | 0.648ms | 1.38x | 6% |
| GPU1 | 0.569ms | 1.21x | 10% |
| GPU2 | **0.900ms** | **1.92x** | **10%** |
| GPU3 | 0.599ms | 1.28x | 4% |

→ tensor parallel에서 특정 GPU(GPU2)에 부하가 집중됨.
`device_map='auto'`가 모델을 균일하게 분배하지 않아서
특정 GPU에 더 많은 레이어/연산이 배치 → 해당 GPU에서 간섭 심화.

#### 32B vs 72B 비교 — 모델 크기가 간섭의 핵심

| | Qwen-32B (16GB/GPU) | Qwen-72B (36GB/GPU) |
|---|---|---|
| HBM 사용률 | ~38% | ~45% |
| 최대 간섭 (4N) | **1.0x** | **1.92x** |
| TTI miss (4N) | **0%** | **10%** |
| NVLink 영향 | 없음 | **있음 (all-reduce bandwidth 추가)** |

**모델이 커서 per-GPU HBM 사용이 높을수록 bandwidth 경쟁이 심해지고 간섭 증가.**
NVLink tensor parallel 통신은 추가적인 bandwidth 부하를 더함.

#### MIG-like 격리 효과 비교

| 설정 | GPT-2 간섭 | 72B TP 간섭 | 비고 |
|------|-----------|------------|------|
| **격리 없음** (MPS 100:100) | **3.70x** | - | Exp-17 |
| **MIG-like 40:60** (MPS SM% 제한) | - | **최대 1.99x** | 본 실험 |

→ **SM 격리만으로 간섭이 3.70x → 1.99x로 감소 (~46% 개선).**
→ 하지만 bandwidth 공유 때문에 완전 제거는 안 됨.
→ 특히 큰 모델(72B)에서 노드 스케일 시 worst-case GPU miss 10%.

#### 결론

```
1. 작은 모델 (32B, 16GB/GPU): MIG-like 격리로 충분 → 간섭 0, 스케일 완벽
2. 큰 모델 (72B, 36GB/GPU): MIG-like 격리로도 간섭 남음 → worst GPU 1.99x, miss 10%
3. 간섭의 원인: HBM bandwidth 공유 + NVLink all-reduce 통신
4. GPU 간 불균형: tensor parallel 분배가 불균일 → 특정 GPU에 간섭 집중
5. 스케일 시 악화: 1N miss 2% → 4N miss 10% (노드 추가가 간섭을 악화시킴)

→ "SM 격리(MIG)는 도움이 되지만, 큰 모델의 bandwidth 문제는 남는다"
→ "추가로 bandwidth-aware scheduling이 필요하다"가 여전히 유효
```
