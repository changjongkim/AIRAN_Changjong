"""
L1 Measurement — Multi-cell to increase GPU utilization.
Runs N independent cuPHY pipeline instances per iteration to simulate
multi-cell gNB (matching NVIDIA PoC paper's 30-40% GPU usage).

Usage: python3 run_l1_multicell.py <label> <num_cells>
"""
import os
import sys
import json
...
def main():
    label = sys.argv[1] if len(sys.argv) > 1 else "multicell"
    ...
    # Use 1T2R per cell (Sionna compatible), scale via num_cells
    num_tx, num_rx, mcs_index = 1, 2, 2

    # Channel (shared across cells — same model, different random realizations)
    ...
    print(f"HBM after setup: {(total-free)/1e9:.1f}/{total/1e9:.1f}GB ({hbm_used_pct:.0f}%)", flush=True)

    # Measure — process all cells per iteration
    ...
    with open(out, "w") as f:
        json.dump({"label": label, "num_cells": num_cells,
