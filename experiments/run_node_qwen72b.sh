#!/bin/bash
# Per-node: 4 GPU, each runs L1, Qwen-72B runs across all 4 GPUs (tensor parallel)
# Args: <label> <cells> <ai_mode>
# ai_mode: none / qwen72b
...
# Start Qwen-72B 4GPU tensor parallel (background)
AI_PID=""
if [ "$AI_MODE" = "qwen72b" ]; then
...
print('[Qwen-72B] Loading model (4GPU TP)...', flush=True)
...
print('[Qwen-72B] Running inference loop...', flush=True)
c = 0; start = time.time()
with torch.no_grad():
    while time.time() - start < 180:
        model(dummy); c += 1
        if c % 10 == 0:
            print(f'[Qwen-72B] {c} iters, {c/(time.time()-start):.1f} it/s', flush=True)
print(f'[Qwen-72B] done: {c} iters', flush=True)
" &
    AI_PID=$!
    echo "[$HOSTNAME] Waiting 120s for Qwen-72B to load..."
    sleep 120
    echo "[$HOSTNAME] Qwen-72B PID=$AI_PID"
fi
