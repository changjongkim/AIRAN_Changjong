...
#SBATCH --exclusive
#SBATCH -J scale_4n
#SBATCH -o /pscratch/sd/s/sgkim/kcj/AI-RAN/jobs/scale_4n_%j.out
#SBATCH -e /pscratch/sd/s/sgkim/kcj/AI-RAN/jobs/scale_4n_%j.err
...
echo "============================================================"
echo "Scale test: 4 Nodes (each: L1 CUDA Graph + AI)"
...
for mode in "baseline none 0" "neuralrx neural_rx 1.0" "gpt2 gpt2 1.0" "resnet resnet 1.0"; do
    ...
    srun --mpi=pmi2 shifter --image=$IMAGE \
        python3 $SCRIPT scale4n_${name} 8 $ai_type $intensity $DUR
    echo ""
done
